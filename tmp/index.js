(function () {
    ...
    _createClass(Dom, [{
      ...
      value: function setClass(className) {
        if (this.$el.className.search(className) < 0) {
          this.$el.className += " ".concat(className);
        }
      }
    }, {
      key: "removeClass",
      value: function removeClass(className) {
        if (this.$el.className.search(className) >= 0) {
          this.$el.className = this.$el.className.replace(" ".concat(className), '');
        }
      }
    }]);
  ...
  /*#__PURE__*/
  function (_Dom) {
    _inherits(BannerSlide, _Dom);

    function BannerSlide(el, index, parent) {
      var _this;

      _classCallCheck(this, BannerSlide);

      _this = _possibleConstructorReturn(this, _getPrototypeOf(BannerSlide).call(this, el));
      _this.parent = parent;
      _this.props = _objectSpread2({
        title: '',
        discount: '',
        date: ''
      }, _this.$el.dataset);
      ...
      _this.timer = setInterval(_this.checkTime.bind(_assertThisInitialized(_this)), 1000);
      return _this;
    }

    _createClass(BannerSlide, [{
      key: "init",
      value: function init() {
        ...
        var dt = this.$el.dataset.actionDate.split(/\s?-\s?/);
        this.startTime = Date.parse("2019.".concat(dt[0], " 00:00:00").replace(/\./g, '/'));
        this.endTime = Date.parse("2019.".concat(dt[1], " 23:59:59").replace(/\./g, '/'));
        this.props.startDate = "2019.".concat(dt[0]).replace(/\./g, '/');
        this.props.endDate = "2019.".concat(dt[1]).replace(/\./g, '/');
      ...
      key: "formatDate",
      value: function formatDate(startDate, endDate) {
        var monthNames = ['января', 'февраля', 'марта', 'апреля', 'мая', 'июня', 'июля', 'августа', 'сентября', 'октября', 'ноября', 'декабря'];
        var sdt = new Date(startDate);
        var edt = new Date(endDate);

        if (sdt.getMonth() == edt.getMonth()) {
          return "".concat(sdt.getDate(), "-").concat(edt.getDate(), " ").concat(monthNames[sdt.getMonth()]);
        } else {
          return "".concat(sdt.getDate(), " ").concat(monthNames[sdt.getMonth()], " - ").concat(edt.getDate(), " ").concat(monthNames[edt.getMonth()]);
        }
      }
    }, {
      key: "checkTime",
      value: function checkTime() {
        ...
        if (this.time >= this.startTime && this.time <= this.endTime) {
          if (!this.started) {
            var event = new CustomEvent('action:ready', {
              bubbles: true,
              detail: {
      ...
      key: "renderTimer",
      value: function renderTimer(currentTime, startTime, endTime) {
        var resDateTime = '';
        var dt;

        if (currentTime < startTime) {
          dt = this.timeLeft(currentTime, startTime);
          resDateTime = "\u0414\u043E \u0441\u0442\u0430\u0440\u0442\u0430: ".concat(this.timeString(dt));
        } else if (currentTime >= startTime && currentTime <= endTime) {
          dt = this.timeLeft(currentTime, endTime);
          resDateTime = "\u0414\u043E \u043A\u043E\u043D\u0446\u0430: ".concat(this.timeString(dt));
        } else {
      ...
      key: "timeString",
      value: function timeString(dt) {
        var formatNum = function formatNum(num) {
          return parseInt(num) > 9 ? "".concat(num) : "0".concat(parseInt(num));
        };

        var res = '';
        if (dt.days) res += "".concat(dt.days, "\u0434. ");
        if (dt.hours) res += "".concat(formatNum(dt.hours), "\u0447. ");
        if (dt.minutes) res += "".concat(formatNum(dt.minutes), "\u043C. ");
        if (dt.seconds) res += "".concat(formatNum(dt.seconds), "\u0441. ");
        return res;
      }
    }, {
      key: "timeLeft",
      value: function timeLeft(startTime, finalTime) {
        var timeLeft = finalTime - startTime; //(new Date()).getTime()
    ...
    _createClass(Banner, [{
      ...
      value: function init() {
        ...
        this.$slides.forEach(function (el, index) {
          _this2.slides.push(new BannerSlide(el, index, _this2));
        });
        this.calcSizes();
      ...
      value: function calcSizes() {
        ...
        this.slides.forEach(function (slide) {
          slide.$el.style.width = "".concat(_this3.slideWidth, "%");
        });
        this.$sliderTrack.style.width = "".concat(this.slidesCount / this.visibleSlidesCount * 250, "%");
      }
    }, {
      key: "setInitialized",
      ...
      value: function getStartPosition() {
        var startPosition = 0; // for (let [key, slide] of this.slides.entries()) {
      ...
      key: "actionStarted",
      value: function actionStarted(event) {
  ...
  document.addEventListener('DOMContentLoaded', function (event) {
    var banner = new Banner('.bf-actions', {
      slides: 4
    });
  }, false);
